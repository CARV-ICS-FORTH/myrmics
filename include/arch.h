...
// Last modified : $Date: 2013/07/05 15:42:51 $
...
// should be ARM0.
//
// For the above restrictions to be met, there are limited options (activate
// ONE of the 6 configurations below): Formic cube alone, Formic cube + ARM0,
// Formic cube + ARM0 + ARM1, or any of these three plus the XUP board.
...
#if 0 // Formic cube + XUP
#define AR_BOOT_MASTER_BID              0x00    // 0x6B = ARM0, 0x00 = Formic
#define AR_ARM0_BID                     -1      // 0x6B = ARM0, -1 = inactive
#define AR_ARM1_BID                     -1      // 0x4B = ARM1, -1 = inactive
#define AR_XUP_BID                      0x57    // 0x57 = XUP,  -1 = inactive
#endif

#if 0 // Formic cube + ARM0 + XUP
#define AR_BOOT_MASTER_BID              0x6B    // 0x6B = ARM0, 0x00 = Formic
#define AR_ARM0_BID                     0x6B    // 0x6B = ARM0, -1 = inactive
#define AR_ARM1_BID                     -1      // 0x4B = ARM1, -1 = inactive
#define AR_XUP_BID                      0x57    // 0x57 = XUP,  -1 = inactive
...
#if 0 // Formic cube + ARM0 + ARM1 + XUP
#define AR_BOOT_MASTER_BID              0x6B    // 0x6B = ARM0, 0x00 = Formic
#define AR_ARM0_BID                     0x6B    // 0x6B = ARM0, -1 = inactive
#define AR_ARM1_BID                     0x4B    // 0x4B = ARM1, -1 = inactive
#define AR_XUP_BID                      0x57    // 0x57 = XUP,  -1 = inactive
...
// For Formic printing, which board's UART is connected to a host terminal
...
#define AR_FORMIC_CORES_PER_BOARD       1

#define AR_FORMIC_MIN_X                 0
#define AR_FORMIC_MIN_Y                 0
#define AR_FORMIC_MIN_Z                 0
#define AR_FORMIC_MAX_X                 0
#define AR_FORMIC_MAX_Y                 0
#define AR_FORMIC_MAX_Z                 0
...
// Which Formic board is connected to the XUP board
...
extern float ar_sin(float x);
extern float ar_cos(float x);
extern float ar_tan(float x);
...
extern unsigned int ar_timer_get_usec();
...
// Send to mailbox one message word, no ack
...
// Send to mailbox two message words, no ack
static inline void ar_mbox_send2(int my_cid, int dst_bid, int dst_cid,
                                 unsigned int msg0, unsigned int msg1) {
#ifdef ARCH_ARM
  *ARS_MNI_MSG0(my_cid)    = msg0;
  *ARS_MNI_MSG1(my_cid)    = msg1;
  *ARS_MNI_DST_ADR(my_cid) = ar_addr_of_mbox(dst_bid, dst_cid);
  *ARS_MNI_OPCODE(my_cid)  = (dst_bid << 20) | (dst_cid << 16) | 0x1;
#endif
#ifdef ARCH_MB
  *MBS_MNI_MSG0    = msg0;
  *MBS_MNI_MSG1    = msg1;
  *MBS_MNI_DST_ADR = ar_addr_of_mbox(dst_bid, dst_cid);
  *MBS_MNI_OPCODE  = (dst_bid << 20) | (dst_cid << 16) | 0x1;
#endif
}
...
// Send to mailslot one message word, no ack
static inline void ar_mslot_send(int my_cid, int dst_bid, int dst_cid,
                                 unsigned int msg) {
#ifdef ARCH_ARM
  *ARS_MNI_MSG0(my_cid)    = msg;
  *ARS_MNI_DST_ADR(my_cid) = ar_addr_of_mslot(dst_bid, dst_cid);
  *ARS_MNI_OPCODE(my_cid)  = (dst_bid << 20) | (dst_cid << 16);
#endif
#ifdef ARCH_MB
  *MBS_MNI_MSG0    = msg;
  *MBS_MNI_DST_ADR = ar_addr_of_mslot(dst_bid, dst_cid);
  *MBS_MNI_OPCODE  = (dst_bid << 20) | (dst_cid << 16);
#endif
}
...
extern void ar_send_xup_arbitrary(int my_bid, int my_cid, unsigned int my_adr,
                                  unsigned int xup_adr, int num_pixels,
                                  int my_cnt);
